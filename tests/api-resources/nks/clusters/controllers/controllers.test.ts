...
describe('resource controllers', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.nks.clusters.controllers.list('cluster_id', { cursor: 'cursor', limit: 10 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('get: only required params', async () => {
    const responsePromise = client.nks.clusters.controllers.get('controller_id', { cluster_id: 'cluster_id' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
