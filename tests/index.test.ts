...
import { APIPromise } from '@nirvana-labs/nirvana/core/api-promise';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['NIRVANA_LABS_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: NirvanaLabs) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new NirvanaLabs({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new NirvanaLabs({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['NIRVANA_LABS_LOG'] = 'debug';
      const client = new NirvanaLabs({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['NIRVANA_LABS_LOG'] = 'not a log level';
      const client = new NirvanaLabs({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'NIRVANA_LABS_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['NIRVANA_LABS_LOG'] = 'debug';
      const client = new NirvanaLabs({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    ...
    test('in request options overridden by client options', () => {
      const client = new NirvanaLabs({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['NIRVANA_LABS_BASE_URL'] = 'http://localhost:5000/env';
      const client = new NirvanaLabs({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
