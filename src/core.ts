...
export class APIPromise<T> extends Promise<T> {
   ...
   * or add one of these imports before your first `import … from '@nirvana-labs/nirvana'`:
...
export function debug(action: string, ...args: any[]) {
  if (typeof process !== 'undefined' && process?.env?.['DEBUG'] === 'true') {
    console.log(`NirvanaLabs:DEBUG:${action}`, ...args);
  }
}
...
export const toBase64 = (str: string | null | undefined): string => {
  ...
  throw new NirvanaLabsError('Cannot generate b64 string; Expected `Buffer` or `btoa` to be defined');
};
