...
export class NirvanaLabs {
   ...
   */
  constructor({
    baseURL = readEnv('NIRVANA_LABS_BASE_URL'),
    apiKey = readEnv('NIRVANA_LABS_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
    ...
    this.timeout = options.timeout ?? NirvanaLabs.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('NIRVANA_LABS_LOG'), 'process.env[\'NIRVANA_LABS_LOG\']', this) ?? defaultLogLevel;
...
export {
  InstanceTypes as InstanceTypes
};
