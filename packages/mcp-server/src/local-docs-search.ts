...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    response:
      '{ daily_cost: string; effective_balance: string; monthly_cost: string; recharge_threshold_fraction: string; estimated_next_charge_at?: string; runway_months?: string; }',
    markdown:
      "## recharge\n\n`client.organizations.billing.recharge(organization_id: string, Idempotency-Key: string): { daily_cost: string; effective_balance: string; monthly_cost: string; recharge_threshold_fraction: string; estimated_next_charge_at?: string; runway_months?: string; }`\n\n**post** `/v1/organizations/{organization_id}/billing/recharge`\n\nCharge the card on file up to the recharge target now instead of waiting for the scheduled retry. Automatic-policy prepaid organizations only. Idempotency-Key header required.\n\n### Parameters\n\n- `organization_id: string`\n\n- `Idempotency-Key: string`\n\n### Returns\n\n- `{ daily_cost: string; effective_balance: string; monthly_cost: string; recharge_threshold_fraction: string; estimated_next_charge_at?: string; runway_months?: string; }`\n  Forward-looking billing summary for an organization. All costs are run-rate projections from the organization's current active usage (\"≈ $X/mo at current usage\").\n\n  - `daily_cost: string`\n  - `effective_balance: string`\n  - `monthly_cost: string`\n  - `recharge_threshold_fraction: string`\n  - `estimated_next_charge_at?: string`\n  - `runway_months?: string`\n\n### Example\n\n```typescript\nimport NirvanaLabs from '@nirvana-labs/nirvana';\n\nconst client = new NirvanaLabs();\n\nconst organizationBillingSummary = await client.organizations.billing.recharge('organization_id', { 'Idempotency-Key': 'Idempotency-Key' });\n\nconsole.log(organizationBillingSummary);\n```",
    perLanguage: {
      typescript: {
