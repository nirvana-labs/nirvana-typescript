...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    params: ['organization_id: string;', 'cursor?: string;', 'limit?: number;'],
    response:
      "{ items: { id: string; amount: string; created_at: string; currency: string; type: billing_history_entry_type; description?: string; funding_purpose?: 'first_charge' | 'auto_recharge' | 'manual_top_up' | 'manual_recharge'; receipt_url?: string; }[]; pagination: { next_cursor: string; previous_cursor: string; total_count: number; }; }",
    markdown:
      "## history\n\n`client.organizations.billing.history(organization_id: string, cursor?: string, limit?: number): { items: billing_history_entry[]; pagination: pagination; }`\n\n**get** `/v1/organizations/{organization_id}/billing/history`\n\nList the organization's billing history: prepaid credits, top-ups, and manual adjustments, newest first. Paginated with an opaque cursor.\n\n### Parameters\n\n- `organization_id: string`\n\n- `cursor?: string`\n  Pagination cursor returned by a previous request\n\n- `limit?: number`\n  Maximum number of items to return\n\n### Returns\n\n- `{ items: { id: string; amount: string; created_at: string; currency: string; type: billing_history_entry_type; description?: string; funding_purpose?: 'first_charge' | 'auto_recharge' | 'manual_top_up' | 'manual_recharge'; receipt_url?: string; }[]; pagination: { next_cursor: string; previous_cursor: string; total_count: number; }; }`\n\n  - `items: { id: string; amount: string; created_at: string; currency: string; type: 'grant' | 'adjustment'; description?: string; funding_purpose?: 'first_charge' | 'auto_recharge' | 'manual_top_up' | 'manual_recharge'; receipt_url?: string; }[]`\n  - `pagination: { next_cursor: string; previous_cursor: string; total_count: number; }`\n\n### Example\n\n```typescript\nimport NirvanaLabs from '@nirvana-labs/nirvana';\n\nconst client = new NirvanaLabs();\n\nconst billingHistoryEntryList = await client.organizations.billing.history('organization_id');\n\nconsole.log(billingHistoryEntryList);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    description:
      "Get the organization's recharge configuration: the top-up mode and the fixed and proportional threshold components.",
