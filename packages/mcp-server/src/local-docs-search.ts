...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    response:
      "{ id: string; asn: number; aws: { region: string; }; bandwidth_mbps: 50 | 200 | 500 | 1000 | 2000; cidrs: string[]; created_at: string; name: string; project_id: string; provider_asn: number; provider_cidrs: string[]; provider_router_ip: string; region: 'us-sva-2'; router_ip: string; status: 'pending' | 'creating' | 'updating' | 'ready' | 'deleting' | 'deleted' | 'error'; tags: string[]; updated_at: string; }",
    markdown:
      "## list\n\n`client.networking.connect.connections.list(project_id: string, bandwidth_mbps?: 50 | 200 | 500 | 1000 | 2000, cursor?: string, limit?: number, name?: string, provider?: string, provider_region?: string, region?: string, sort?: string, status?: 'pending' | 'creating' | 'updating' | 'ready' | 'deleting' | 'error', tags?: string[]): { id: string; asn: number; aws: connect_connection_aws_config; bandwidth_mbps: connect_bandwidth_mbps; cidrs: string[]; created_at: string; name: string; project_id: string; provider_asn: number; provider_cidrs: string[]; provider_router_ip: string; region: region_name; router_ip: string; status: resource_status; tags: string[]; updated_at: string; }`\n\n**get** `/v1/networking/connect/connections`\n\nList all Connect Connections\n\n### Parameters\n\n- `project_id: string`\n  Project ID of resources to request\n\n- `bandwidth_mbps?: 50 | 200 | 500 | 1000 | 2000`\n  Filter by provisioned bandwidth in Mbps\n\n- `cursor?: string`\n  Pagination cursor returned by a previous request. Only valid for the same filters and sort order.\n\n- `limit?: number`\n  Maximum number of items to return\n\n- `name?: string`\n  Filter by a case-insensitive substring of the Connect Connection name\n\n- `provider?: string`\n  Filter by provider\n\n- `provider_region?: string`\n  Filter by the provider's own region\n\n- `region?: string`\n  Filter by Nirvana region\n\n- `sort?: string`\n  Comma-separated sort terms in precedence order, each field:asc or field:desc. Fields: created_at, updated_at, name, status, bandwidth_mbps\n\n- `status?: 'pending' | 'creating' | 'updating' | 'ready' | 'deleting' | 'error'`\n  Filter by Connect Connection status\n\n- `tags?: string[]`\n  Filter by tags. Repeat the parameter to require several tags; a Connect Connection must carry all of them.\n\n### Returns\n\n- `{ id: string; asn: number; aws: { region: string; }; bandwidth_mbps: 50 | 200 | 500 | 1000 | 2000; cidrs: string[]; created_at: string; name: string; project_id: string; provider_asn: number; provider_cidrs: string[]; provider_router_ip: string; region: 'us-sva-2'; router_ip: string; status: 'pending' | 'creating' | 'updating' | 'ready' | 'deleting' | 'deleted' | 'error'; tags: string[]; updated_at: string; }`\n  Connect Connection details.\n\n  - `id: string`\n  - `asn: number`\n  - `aws: { region: string; }`\n  - `bandwidth_mbps: 50 | 200 | 500 | 1000 | 2000`\n  - `cidrs: string[]`\n  - `created_at: string`\n  - `name: string`\n  - `project_id: string`\n  - `provider_asn: number`\n  - `provider_cidrs: string[]`\n  - `provider_router_ip: string`\n  - `region: 'us-sva-2'`\n  - `router_ip: string`\n  - `status: 'pending' | 'creating' | 'updating' | 'ready' | 'deleting' | 'deleted' | 'error'`\n  - `tags: string[]`\n  - `updated_at: string`\n\n### Example\n\n```typescript\nimport NirvanaLabs from '@nirvana-labs/nirvana';\n\nconst client = new NirvanaLabs();\n\n// Automatically fetches more pages as needed.\nfor await (const connectConnection of client.networking.connect.connections.list({ project_id: 'project_id' })) {\n  console.log(connectConnection);\n}\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response: "{ nirvana_region: 'us-sva-2'; provider: string; provider_region: string; }",
    markdown:
      "## list\n\n`client.networking.connect.routes.list(cursor?: string, limit?: number, provider?: string, provider_region?: string, region?: string, sort?: string): { nirvana_region: region_name; provider: string; provider_region: string; }`\n\n**get** `/v1/networking/connect/routes`\n\nList all supported routes with regions for Connect.\n\n### Parameters\n\n- `cursor?: string`\n  Pagination cursor returned by a previous request. Only valid for the same filters and sort order.\n\n- `limit?: number`\n  Maximum number of items to return\n\n- `provider?: string`\n  Filter by provider\n\n- `provider_region?: string`\n  Filter by the provider's own region\n\n- `region?: string`\n  Filter by the Nirvana region the route reaches\n\n- `sort?: string`\n  Comma-separated sort terms in precedence order, each field:asc or field:desc. Fields: created_at, provider_region, region\n\n### Returns\n\n- `{ nirvana_region: 'us-sva-2'; provider: string; provider_region: string; }`\n  Routes supported for Connect.\n\n  - `nirvana_region: 'us-sva-2'`\n  - `provider: string`\n  - `provider_region: string`\n\n### Example\n\n```typescript\nimport NirvanaLabs from '@nirvana-labs/nirvana';\n\nconst client = new NirvanaLabs();\n\n// Automatically fetches more pages as needed.\nfor await (const connectRoute of client.networking.connect.routes.list()) {\n  console.log(connectRoute);\n}\n```",
    perLanguage: {
      typescript: {
