...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { NIRVANA_LABS_API_KEY: requireValue(readEnv('NIRVANA_LABS_API_KEY') ?? client.apiKey, 'set NIRVANA_LABS_API_KEY environment variable or provide apiKey client option'), NIRVANA_LABS_BASE_URL: readEnv('NIRVANA_LABS_BASE_URL') ?? client.baseURL ?? undefined };
